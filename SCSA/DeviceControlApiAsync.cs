...
namespace SCSA
{
    public class DeviceControlApiAsync : IDisposable
    {
        private readonly DeviceControlSetting _deviceControlSetting;
        private readonly ITcpClient<NetDataPackage> _tcpClient;
        private Int16 _flag;

        public event Action<Dictionary<Parameter.DataChannelType, double[,]>> DataReceived;

        private readonly ConcurrentDictionary<DeviceCommand, List<TaskCompletionSource<NetDataPackage>>>
            _pendingCommands;

        public DeviceControlApiAsync(ITcpClient<NetDataPackage> tcpClient)
        {
            _tcpClient = tcpClient;
            tcpClient.DataReceived += _tcpClient_DataReceived;
            _pendingCommands = new ConcurrentDictionary<DeviceCommand, List<TaskCompletionSource<NetDataPackage>>>();
        }

        public async Task<NetDataPackage> SendAsync(DeviceCommand command, byte[] data,
            CancellationToken cancellationToken)
        {
            if (_tcpClient == null || !_tcpClient.Connected.GetValueOrDefault(true))
                return null;

            var netPackage = new NetDataPackage();
            netPackage.Flag = _flag++;
            if (_flag > 2048)
                _flag = 0;
            ...
            var tcs = new TaskCompletionSource<NetDataPackage>();

            // 添加到 List 中
            var tcsList = _pendingCommands.GetOrAdd(netPackage.DeviceCommand,
                _ => new List<TaskCompletionSource<NetDataPackage>>());
            lock (tcsList)
            {
                tcsList.Add(tcs);
            }
            ...
            // 发送消息失败，移除 TCS
            if (!_tcpClient.SendMessage(netPackage))
            {
                lock (tcsList)
                {
                    tcsList.Remove(tcs);
        ...
        private Dictionary<Parameter.DataChannelType, double[,]> ProcessChannelData(byte[] data)
        {
            ...
            var channelType = (Parameter.DataChannelType)reader.ReadByte();
        ...
        private void _tcpClient_DataReceived(object? sender, INetDataPackage e)
        {
            var netDataPackage = e as NetDataPackage;
            if (netDataPackage == null)
                return;
