...
{
    public static readonly Dictionary<string, OxyImage> IconCache = new();
    ...
    {
        // 计算渲染尺寸（建议至少为字体大小的3倍）
        var renderSize = Math.Max(fontSize * 3, 64);
        var pixelSize = (int)Math.Ceiling(renderSize);
        ...
        {
            // 获取字体资源
            var iconFont = new FontFamily("avares://SCSA.Plot/Assets/Fonts/iconfont.ttf#iconfont");
            ...
            // 计算缩放比例并留10%的边距
            var widthRatio = renderSize / testText.Width;
            var heightRatio = renderSize / testText.Height;
